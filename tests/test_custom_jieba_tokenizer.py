...
class CustomJiebaTokenizerTestCase(TestCase):
    ...
    def test_lcut_sentences_parallel(self):
        ...
        tokenizer = CustomJiebaTokenizer(HMM=False)
    ...
    def test_without_hmm(self):
        self.assertEqual(
            ['我', '想', '喝', '珍珠奶茶', '半', '糖', '大', '杯'],
            self.tokenizer.lcut('我想喝珍珠奶茶半糖大杯', HMM=False),
        )
        tokenizer = CustomJiebaTokenizer(HMM=False)
        self.assertEqual(
            self.tokenizer.lcut('我想喝珍珠奶茶半糖大杯', HMM=False),
    ...
    def test_cut_all(self):
        ...
        tokenizer = CustomJiebaTokenizer(cut_all=False)
        self.assertEqual(
            self.tokenizer.lcut('我想喝珍珠奶茶半糖大杯', cut_all=False),
            tokenizer.lcut('我想喝珍珠奶茶半糖大杯'),
        )
